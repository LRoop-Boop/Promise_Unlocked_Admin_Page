...
export default function ReportsPage({ students }: ReportsPageProps) {
  ...
  const acceptanceRate     = decidedApplicants > 0 ? Math.round((acceptedCount / decidedApplicants) * 100) : 0;
  const avgGpa             = (students.reduce((sum, s) => sum + s.gpa, 0) / totalApplicants).toFixed(2);
  const mostPopularProgram = Object.entries(
    students.reduce((acc, s) => { acc[s.program] = (acc[s.program] ?? 0) + 1; return acc; }, {} as Record<string, number>)
